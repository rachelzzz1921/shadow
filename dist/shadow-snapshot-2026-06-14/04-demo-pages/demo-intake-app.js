...
(function intakeApp() {
  ...
  async function goDemoFromPreset(presetId) {
    ...
    try {
      ...
      window.location.href = window.ShadowDemoMock.demoBrowseHref(presetId);
    } catch (e) {
  ...
  async function submitIntake() {
    ...
    if (el.previewBody) {
      el.previewBody.textContent = result.persona?.core_tension ||
        '人格已就绪，正在进入七年 Live 生成…';
    }
    el.btnNext.textContent = '进入七年生成…';
    window.location.href = 'demo-live.html?autostart=1';
  }
