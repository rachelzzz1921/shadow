    ...
    class ShadowLayoutScene extends Phaser.Scene {
      create() {
        ...
        (layout.layers || []).forEach((layer, i) => {
          ...
          const y = layer.y ?? 200;
          const box = this.add.rectangle(x, y, 48, 64, 0x445566, 0.55);
          this.add.text(x - 24, y + 36, layer.role || 'layer', {
            fontSize: '9px', color: '#9999aa'
          });
          this.add.text(x - 24, y - 44, (layer.asset_id || '').replace('PX-', ''), {
            fontSize: '8px', color: '#777788'
          });
        });
