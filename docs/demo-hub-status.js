  ...
  function showPagesPreviewHint() {
    setState(
      ...
      '● GitHub Pages 预览 · 下方四条故事 <a href="demo.html?story=linwan">Mock 秒开</a> · ' +
        '<a href="intake.html">Intake</a> 可规则合成七年 · 真 LLM 生成：本地 <code>npm run demo:local</code> → ' +
        '<a href="http://localhost:3000/demo-hub.html">localhost:3000</a>'
    );
    if (ctaFoot) {
      ctaFoot.textContent =
    ...
    .then(d => {
      ...
      if (d.has_key) {
        ...
        const intakeHint = hasIntake
          ? ' · 已采集人格 · <a href="intake.html">继续生成</a>'
          : ' · <a href="intake.html">写岔路口</a> 可走真生成';
        setState('is-ok', `● API 已连接 · ${llm} · ${rag}${intakeHint}`);
        return;
      }
      setState(
        'is-warn',
        `● 服务在线 · Mock 模式（未配置 Key）· ${rag} · 预设故事可秒开 · <a href="intake.html">Intake</a> 走规则合成`
      );
    })
    .catch(() => {
