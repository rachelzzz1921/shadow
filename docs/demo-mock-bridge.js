...
(function exportDemoMock(global) {
  const DEMO_STORY_IDS = new Set(['wufuxdu', 'fuxduxian', 'linwan', 'heartbeat_line', 'zhoudran']);

  const STORY_FILES = {
    wufuxdu: 'stories/未复读线-阿岚.json',
  ...
  const CATALOG = {
    wufuxdu: { id: 'wufuxdu', line_name: '未复读线', scenario_primary: 'self_growth', scenario_secondary: 'friendship' },
    fuxduxian: { id: 'fuxduxian', line_name: '旧复读线', scenario_primary: 'academic' },
  ...
  function shouldUseDemoMock() {
    if (sessionStorage.getItem('shadow_custom_intake') === '1') return false;
  ...
  function markCustomIntake() {
    sessionStorage.setItem('shadow_custom_intake', '1');
    clearDemoMode();
  }

  function clearCustomIntake() {
    sessionStorage.removeItem('shadow_custom_intake');
  }
   ...
   */
  async function completeIntakeMock(storyId, ctx, opts = {}) {
    ...
    if (!opts.quick && !visual_character && global.ShadowCharacterLibrary?.matchLocal) {
  ...
  function demoBrowseHref(storyId) {
    if (!storyId || storyId === 'wufuxdu') return 'demo.html?live=1&from=intake';
    return `demo.html?live=1&from=intake&story=${encodeURIComponent(storyId)}`;
  }
