...
(function exportIntakeQuestions(global) {
  ...
  function renderRank(q, value, onChange, onCommit) {
    const ordered = [...(value?.orderedKeys || [])];
    ...
    for (const opt of q.options || []) {
      const pos = ordered.indexOf(opt.key);
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'intake-opt' + (pos >= 0 ? ' active' : '');
      btn.innerHTML = `<span class="intake-opt-key">${pos >= 0 ? pos + 1 : '—'}</span><span class="intake-opt-text">${opt.text}</span>`;
      btn.addEventListener('click', () => {
        let next;
        if (pos >= 0) next = ordered.filter((k) => k !== opt.key);
        else next = [...ordered, opt.key];
        onChange({ orderedKeys: next });
        if (next.length === (q.options?.length || 0)) {
          setTimeout(() => onCommit?.(), AUTO_MS);
        }
      });
