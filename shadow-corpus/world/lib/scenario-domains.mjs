...
export const KEYWORD_SCENARIO = [
  { re: /复读|高考|考研|学业|学校|专业|留学|保研|四六级|转专业|出国读书|出国/, domain: 'academic', boost: 0.35 },
  { re: /父母|家里|母亲|父亲|亲情|愧疚|责任|期待|家庭/, domain: 'family', boost: 0.35 },
  { re: /分手|复合|恋爱|喜欢|暧昧|相亲|异地|结婚|感情|亲密关系/, domain: 'love', boost: 0.35 },
  { re: /朋友|室友|圈子|孤独|合群|疏远|分流|陪伴/, domain: 'friendship', boost: 0.3 },
  { re: /工作|就业|实习|裁员|创业|考公|编制|996|转行|城市机会/, domain: 'career', boost: 0.35 },
  { re: /迷茫|身份|成长|遗憾|自尊|要强|不甘|怕.*看穿|舒适区|人生方向|长期遗憾/, domain: 'self_growth', boost: 0.35 }
];
...
/**
 * 判断用户输入（choice / 问题）属于哪个场景域。
 * @param {string} text — 主文本（人生选择或对话问题）
 * @param {object} [opts]
 * @param {string[]} [opts.keywords]
 * @param {string} [opts.description]
 */
export function classifyUserInput(text, opts = {}) {
  ...
  if (!blob.trim()) {
    const weights = uniformWeights();
    const [top] = topDomains(weights, 1);
    return {
      domain: top.domain,
      label: top.label,
      agent: `${top.label} Agent`,
      brief: SCENARIO_BRIEFS[top.domain],
      confidence: 0,
      scenario_primary: top.domain,
      scenario_secondary: null,
  ...
  for (const { re, domain, boost } of KEYWORD_SCENARIO) {
    if (re.test(blob)) {
      ...
      hits.push({ domain, label: SCENARIO_LABELS[domain] });
    }
  }

  const weights = normalizeWeights(raw);
  const ranked = topDomains(weights, 2);
  const top = ranked[0];
  const second = ranked[1];
  const confidence = Math.max(0, (top?.weight || 0) - (second?.weight || 0));
